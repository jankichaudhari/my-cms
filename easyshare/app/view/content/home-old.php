...
<!--<div id="gallery">
<h1>Easyshare</h1>
	<?php for($i=0;$i<count($display_res);$i++) 
	{ 
		$list_id=$display_res[$i]['type_id'];
		$photo_id=$display_res[$i]['id'];
		?>
   <img src="<?php echo $display_res[$i]['file_path']; ?>" alt="Easyshare" width="116" height="86" border="0" name="<?php echo $photo_id; ?>" id="pic<?php echo $i; ?>" style="opacity:0.7;filter:alpha(opacity=70)" onmouseover='pic<?php echo $i; ?>.style.opacity=0.4,pic<?php echo ($i + 1); ?>.style.opacity=1,pic<?php echo ($i + 2); ?>.style.opacity=0.4;pic<?php echo $i; ?>.filters.alpha.opacity=40,pic<?php echo ($i + 1); ?>.filters.alpha.opacity=100,pic<?php echo ($i + 2); ?>.filters.alpha.opacity=40' onmouseout='pic<?php echo $i; ?>.style.opacity=0.7,pic<?php echo ($i + 1); ?>.style.opacity=0.7,pic<?php echo ($i + 2); ?>.style.opacity=0.7;pic<?php echo $i; ?>.filters.alpha.opacity=70,pic<?php echo ($i + 1); ?>.filters.alpha.opacity=70,pic3.filters.alpha.opacity=70' onclick="index.php?process=display_home&param1=<?php echo $photo_id; ?>" class="ajax-link">
   <?php 
   }
   ?>
	  	</div>
